...
class v2 extends Controller
{
	...
	public function deleterole()
	{
		...
		// get class
		$class = $this->DB->getSelectTB("db_subs_class", "role", $this->e(strtoupper($params['role'])), true);
	...
	public function newmajor()
	{
		...
		$major = $this->DB->costumeDB("
			SELECT * FROM db_class WHERE name='".$this->e($params['major'])."' OR code='".$code."'
			");
		...
		// insert table
		$insert = $this->DB->insertTB("db_class", $dataTable);
	...
	public function changemajor()
	{
		...
		// Get major
		$role = $this->DB->getSelectTB("db_class", "name", $this->e($params['major']), true);

		// chek name major
		if ($role) $this->printJson($this->invalid(false, 403, "This major already exist!"));
		
		// Get major
		$roleCode = $this->DB->getSelectTB("db_class", "code", $this->e($params['code']), true);
		...
		// Update table
		$updateRole = $this->DB->updateTB("db_class", $dataTable, "code", $this->e($params['code']));
	...
	public function deletemajor()
	{
		...
		// Get major
		$major = $this->DB->getSelectTB("db_class", "code", $this->e(strtoupper($params['major'])), true);

		// valid get major
		if (!$major) $this->printJson($this->invalid(false, 403, "This major isn't registered!"));

		// get class
		$class = $this->DB->getSelectTB("db_subs_class", "code", $this->e(strtoupper($params['major'])), true);

		// valid get class
		if ($class) $this->printJson($this->invalid(false, 403, "Unable to delete major. Make sure not to connect with the class!"));

		// Delte table
		$delt = $this->DB->deltTB("db_class", "code", $this->e(strtoupper($params['major'])));
	...
	public function newuser()
	{
		...
		$datatable = [
			...
			"password"	=> $this->e($this->w3llEncode(password_hash("12345678", PASSWORD_DEFAULT))),
	...
	public function changeuser()
	{
		...
		$cls = $this->DB->costumeDB("
			...
			sclass.name AS class
			FROM db_subs_class AS sclass
			INNER JOIN db_class AS class ON class.code = sclass.code
			INNER JOIN db_roles AS role ON role.code = sclass.role
			WHERE 
			sclass.class_code='". $this->e($params['class']) ."'
			AND sclass.role='". $this->e($params['role']) ."'
	...
	public function newclass()
	{
		...
		// get class
		$class = $this->DB->getSelectTB("db_subs_class", "name", $this->e(strtoupper($params['class'])));
		// get major
		$major = $this->DB->getSelectTB("db_class", "code", $this->e(strtoupper($params['major'])));
		// get role
		...
		// insert table
		$execute = $this->DB->insertTB("db_subs_class", $dataTable);
	...
	public function changeclass()
	{
		...
		// get class by code
		$class = $this->DB->getSelectTB("db_subs_class", "class_code", $this->e(strtoupper($params['code'])), true);
		// get class by name
		$className = $this->DB->getSelectTB("db_subs_class", "name", $this->e(strtoupper($params['class'])), true);
		// get major
		$major = $this->DB->getSelectTB("db_class", "code", $this->e(strtoupper($params['major'])));
		// get role
		$role = $this->DB->getSelectTB("db_roles", "code", $this->e(strtoupper($params['role'])));
		...
		// update database
		$execute = $this->DB->updateTB("db_subs_class", $dataTable, "class_code", $this->e($params['code']));
	...
	public function deleteclass()
	{
		...
		// get class
		$class = $this->DB->getSelectTB("db_subs_class", "class_code", $this->e($params['class']),true);
		// get user
		...
		// Delte table
		$execute = $this->DB->deltTB("db_subs_class", "class_code", $this->e(strtoupper($params['class'])));
	...
	public function resetvotes()
	{
		...
		// update table
		$execute = $this->DB->costumeDB("UPDATE db_users SET status='0', take=''");
